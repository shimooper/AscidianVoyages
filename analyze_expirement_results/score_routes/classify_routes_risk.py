...
def run_one_config(config, routes_train_path, routes_test_path, aggregation_type, route_classifier, output_dir):
    ...
    for dataset_name, dataset_df in [('train', train_df), ('test', test_df)]:
        ...
        dataset_df['death_prediction_probability'] = routes_death_predictions_probabilities.round(4)
    ...
    palette = sns.color_palette("Greys", n_colors=len(metrics_df.index))
    sns.barplot(data=df_melted, x='metric', y='value', hue='split', palette=palette)
    plt.xlabel('Metric', fontsize=14)
    plt.ylabel('Score', fontsize=14)
    plt.legend(title='Dataset', bbox_to_anchor=(1.05, 0.5), loc='best', borderaxespad=0.,  title_fontsize=14, fontsize=12)
...
def main(routes_train_path, routes_test_path, config_path, output_dir):
    ...
    ] if not FINAL else [('lr', LogisticRegression(random_state=config.random_state))]
    aggregation_types = ['min', 'max', 'mean', 'log_multiply'] if not FINAL else ['log_multiply']
